/**
 * Flix IPTVs Languages JavaScript
 * 
 * Handles client-side functionality for the Flix IPTVs language management including
 * inline editing of language strings and dynamic table loading.
 * 
 * @package Cockpit
 * @subpackage Flix IPTVs
 */
...
var handleEditableFieldConstruct = function () {
    ...
    if (typeof editableIds !== 'undefined' && editableIds.length > 0) {
        editableIds.forEach(function (id) {
            // Skip invalid IDs that contain spaces or special characters
            if (id.includes(' ') || id.includes('=>') || id.includes('>')) {
                console.warn('Skipping invalid ID:', id);
                return;
            }
            $('#' + id).editable({
                url: './includes/post_flixiptv_languages.php',
                type: 'text',
                pk: function () {
                    return $(this).data('pk');
                },
...
$(function () {
    ...
    $("#lang_select").on("change", function () {
        ...
        $.ajax({
            url: "./includes/get_flixiptv_languages.php",
            type: "POST",
            data: {
