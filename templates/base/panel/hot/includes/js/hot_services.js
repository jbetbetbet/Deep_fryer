/**
 * Hot Player Services JavaScript
 * 
 * Handles client-side functionality for the Hot Player services management including
 * form submissions, AJAX requests, and UI interactions.
 * 
 * @package Cockpit
 * @subpackage Hot Player
 */
...
$(function () {
    ...
    $("#post_settings").on("submit", function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "./includes/post_hot_services.php",
            data: formData,
            processData: false,
            contentType: false,
            success: function () {
    ...
    // Store Info toggle
    $("#storeInfoEnabled").on("change", function () {
        if ($(this).is(':checked')) {
            $("#storeInfoFields").slideDown();
        } else {
            $("#storeInfoFields").slideUp();
        }
    });
 ...
 */
function editClient(mac_address, service_id, username, password, status) {
